...
void draw_bboxes_show(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes, object_rect effect_roi)
{
    static const char* class_names[] = { "person", "bicycle", "car", "motorcycle", "airplane", "bus",
                                        "train", "truck", "boat", "traffic light", "fire hydrant",
                                        "stop sign", "parking meter", "bench", "bird", "cat", "dog",
...
void draw_bboxes_show_human(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes, object_rect effect_roi)
{
    static const char* class_names[] = { "person", "bicycle", "car", "motorcycle", "airplane", "bus",
                                        "train", "truck", "boat", "traffic light", "fire hydrant",
                                        "stop sign", "parking meter", "bench", "bird", "cat", "dog",
...
int image_demo_show(NanoDet &detector, cv::Mat & in, cv::Mat & out)
{
    ...
    object_rect effect_roi;
    cv::Mat resized_img;
    resize_uniform(in, resized_img, cv::Size(320, 320), effect_roi);
    auto results = detector.detect(resized_img, 0.45, 0.5);
    draw_bboxes_show_human(out, results, effect_roi);
    //cv::waitKey(0);
...
int image_human_rect(NanoDet &detector, cv::Mat & in, std::vector<cv::Rect> & recVec, std::vector<float> & scores)
{
    recVec.clear();
    scores.clear();
    if (in.empty())
    ...
    {
        const BoxInfo& bbox = results[i];
        if(bbox.label == 0)
        {
            cv::Rect rect = cv::Rect(cv::Point((bbox.x1 - effect_roi.x) * width_ratio, (bbox.y1 - effect_roi.y) * height_ratio),
